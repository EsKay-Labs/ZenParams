...
document.addEventListener("DOMContentLoaded", () => {
  ...
  status.textContent = "JS FILE LOADED v9.1 (Logic Fix)";
    ...
    fetch("data_bridge.json?t=" + Date.now())
      ...
      .then((data) => {
        if (data.type === "init_all") {
          ...
          // FIX: Apply restoration logic here too!
          if (data.content.current_preset) {
            window.restoreState(data.content.current_preset);
            clearInterval(pollInterval); // Success! Stop polling.
          } else {
            // Only stop polling if we are sure it's done
  ...
  // -------------------------------------------------------------------------
  // HELPER: Restore State (Nuclear Option)
  // -------------------------------------------------------------------------
